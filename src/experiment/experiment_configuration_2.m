...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Experiments for  Configuration 2    %
...
for h = 1:length(hiddenUnitsList)
    for s = 1:size(splits, 1)
        ...
        for run = 1:numRuns
            ...
            % MLP
            mlp = MLP(hiddenUnitsList(h),'logsig', 'softmax', 'crossentropy');

            % Set training parameters: Only for Configuration 2
            mlp.network.trainFcn = 'trainscg';
            mlp.network.trainParam.lr = 0.5;
...
writematrix(results, 'results/report_experiment_configuration_2.csv')